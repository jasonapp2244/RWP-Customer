import 'package:flutter/material.dart';
 
class AppThemData {
  // =========================
  // Brand Colors (From Logo)
  // =========================
  static const Color primaryBlue = Color(0xFF143A59);   // Dark Navy Blue
  static const Color primaryRed = Color(0xFFC92214);    // Strong Red
  static const Color primaryCream = Color(0xFFF2EFEB);  // Off-White
  static const Color warmTint = Color(0xFFC68F8C);      // Light warm accent
  static const Color blue = Color(0xFF016AA2);  
  static const Color blueLight = Color(0xFF0BA4EB);
  static const Color blueLight07 = Color(0xFF0BA4EB);
  static const Color primary950 = Color(0xFF331D00);static const Color secondary950 = Color(0xFF331000);
  // Replacing your old primary range with Blue scale
  static const Color primary50  = Color(0xFFE6ECF1);
  static const Color primary100 = Color(0xFFC2D1DE);
  static const Color primary200 = Color(0xFF9AB3C8);
  static const Color primary300 = Color(0xFF7295B2);
  static const Color primary400 = Color(0xFF527FA2);
  static  Color primary500 = Color(0xFF39698E);  // mid-tone blue
  static const Color primary600 = Color(0xFF2F5877);
  static const Color primary700 = Color(0xFF254661);
  static const Color primary800 = Color(0xFF1C354B);
  static const Color primary900 = Color(0xFF142637);
 
  // Replacing secondary range with RED scale
  static const Color secondary50  = Color(0xFFFFE7E5);
  static const Color secondary100 = Color(0xFFFFC6C1);
  static const Color secondary200 = Color(0xFFFFA49B);
  static const Color secondary300 = Color(0xFFFF8275);
  static const Color secondary400 = Color(0xFFFF5D50);
  static const Color secondary500 = Color(0xFFC92214);   // From logo
  static const Color secondary600 = Color(0xFFB61F12);
  static const Color secondary700 = Color(0xFF9C1B10);
  static const Color secondary800 = Color(0xFF81160D);
  static const Color secondary900 = Color(0xFF61110A);
 
static const Color danger_500p = Color(0xFFFF3B30);


  static const Color success50 = Color(0xFFf0fdf1);
  static const Color success100 = Color(0xFFddfbe1);
  static const Color success200 = Color(0xFFbdf5c6);
  static const Color success300 = Color(0xFF89ec9a);
  static const Color success400 = Color(0xFF4cd964);
  static const Color success500 = Color(0xFF27c041);
  static const Color success600 = Color(0xFF1a9f31);
  static const Color success700 = Color(0xFF187d2a);
  static const Color success800 = Color(0xFF186326);
  static const Color success900 = Color(0xFF165122);
  static const Color success950 = Color(0xFF062d0e);


  // =========================
  // Basic Colors
  // =========================
  static const Color white = Color(0xFFFFFFFF);
  static const Color black = Color(0xFF131416);
  static const Color bgScreen = Color(0xFFF2EFEB); // From logo cream background
 
  // =========================
  // Status Colors
  // =========================
  static const Color success = Color(0xFF27C041);
  static const Color success02 = Color(0xFFE8F9ED);
  static const Color success07 = Color(0xFF12B669);
  static const Color success08 = Color(0xFF039754);
 
  static const Color error02 = Color(0xFFFFEAEA);
  static const Color error07 = Color(0xFFE24A40);
  static const Color error08 = Color(0xFFC92214); // same red as branding
 
  static const Color warning03 = Color(0xFFFFF4CC);
  static const Color warning06 = Color(0xFFFFD24C);
  static const Color warning08 = Color(0xFFE6B100);
 
  // Info (blue tone)
  static const Color info500 = Color(0xFF1EADFF);
  static const Color info50 = Color(0xFFEAF7FF);
  static const Color info950 = Color(0xFF0E315D);
 
  // =========================
  // Greys (Keep your scale)
  // =========================
  static const Color grey08 = Color(0xFF4B5565);
 
  static const Color grey25 = Color(0xFFFFFFFF);
  static const Color grey50 = Color(0xFFF5F6F6);
  static const Color grey100 = Color(0xFFE4E7E9);
  static const Color grey200 = Color(0xFFCCD0D5);
  static const Color grey300 = Color(0xFFA8B0B8);
  static const Color grey400 = Color(0xFF7D8793);
  static const Color grey500 = Color(0xFF626C78);
  static const Color grey600 = Color(0xFF545B66);
  static const Color grey700 = Color(0xFF454A52);
  static const Color grey800 = Color(0xFF40444A);
  static const Color grey900 = Color(0xFF383A41);
  static const Color grey950 = Color(0xFF232529);
 
  // =========================
  // Success Gradient (Example)
  // =========================
  static const List<Color> gradient03 = [
    Color(0xFFC92214),
    Color(0xFF143A59),
  ];
 
  // =========================
  // Intro Colors (Adjusted to new brand style)
  // =========================
  static const Color intro1 = Color(0xFFF2EFEB);
  static const Color intro2 = Color(0xFFE6ECF1);
  static const Color intro3 = Color(0xFFFFE7E5);
 
  // =========================
  // Booking Status Colors
  // =========================
  static const Color bookingNew = primaryBlue;
  static const Color bookingOngoing = Color(0xFFFFD24C);
  static const Color bookingCompleted = Color(0xFF27C041);
  static const Color bookingRejected = Color(0xFFC92214);
  static const Color bookingCancelled = Color(0xFFC92214);


  static const Color danger500 = Color(0xFFff3b30);
  static const Color warning500 = Color(0xFF143A59);
  static const Color danger950 = Color(0xff4B0804);
  static const Color danger50 = Color(0xffFFF2F1);



}










// import 'package:flutter/material.dart';

// class AppThemData {
//   static const Color warning03 = Color(0xFFFEF0C7);
//   static const Color warning06 = Color(0xFFFDB022);
//   static const Color warning08 = Color(0xFFDC6803);

//   static const Color blueLight = Color(0xFF0BA4EB);
//   static const Color blueLight07 = Color(0xFF0BA4EB);
//   static const Color blueLight01 = Color(0xFF36BFF9);
//   static const Color white = Color(0xFFFFFFFF);
//   static const Color info500 = Color(0xFF1EADFF);
//   static const Color bgScreen = Color(0xFFF7F8F8);


//   static const Color black = Color(0xFF131416);
//   static const Color error02 = Color(0xFFFEF3F2);
//   static const Color error08 = Color(0xFFD92C20);
//   static const Color error07 = Color(0xFFF04437);
//   static const Color danger50 = Color(0xFFFFF4F0);
//   static const Color success = Color(0xFF27C041);
//   static const Color success02 = Color(0xFFECFDF3);
//   static const Color success07 = Color(0xFF12B669);
//   static const Color success08 = Color(0xFF039754);
//   static const Color labelColorLightPrimary = Color(0xFF000000);
//   static const Color warning500 = Color(0xFFFFD600);
//   static const Color blue = Color(0xFF016AA2);
//   static const Color danger_500p = Color(0xFFFF3B30);

//   static const List<Color> gradient03 = [Color(0xFFFF8008), Color(0xFFFFC837)];

//   static const Color primary50 = Color(0xFFFFF8F0);
//   static const Color primary100 = Color(0xFFFFE9CC);
//   static const Color primary200 = Color(0xFFFFDEB3);
//   static const Color primary300 = Color(0xFFFEC376);
//   static const Color primary400 = Color(0xFFFEB24D);
//   static Color primary500 = const Color(0xFFFEA735);
//   static const Color primary600 = Color(0xFFFE9001);
//   static const Color primary700 = Color(0xFFCB7401);
//   static const Color primary800 = Color(0xFF985701);
//   static const Color primary900 = Color(0xFF653A01);
//   static const Color primary950 = Color(0xFF331D00);

//   static const Color secondary50 = Color(0xFFfff3f1);
//   static const Color secondary100 = Color(0xFFffe4df);
//   static const Color secondary200 = Color(0xFFffcec5);
//   static const Color secondary300 = Color(0xFFffac9d);
//   static const Color secondary400 = Color(0xFFff7c64);
//   static const Color secondary500 = Color(0xFFFE7235);
//   static const Color secondary600 = Color(0xFFFE4E01);
//   static const Color secondary700 = Color(0xFFCB3E01);
//   static const Color secondary800 = Color(0xFF982F01);
//   static const Color secondary900 = Color(0xFF651F01);
//   static const Color secondary950 = Color(0xFF331000);

//   static const Color grey08 = Color(0xFF4B5565);

//   static const Color grey25 = Color(0xFFffffff);
//   static const Color grey50 = Color(0xFFf5f6f6);
//   static const Color grey100 = Color(0xFFe4e7e9);
//   static const Color grey200 = Color(0xFFccd0d5);
//   static const Color grey300 = Color(0xFFa8b0b8);
//   static const Color grey400 = Color(0xFF7d8793);
//   static const Color grey500 = Color(0xFF626c78);
//   static const Color grey600 = Color(0xFF545b66);
//   static const Color grey700 = Color(0xFF454a52);
//   static const Color grey800 = Color(0xFF40444a);
//   static const Color grey900 = Color(0xFF383a41);
//   static const Color grey950 = Color(0xFF232529);

//   static const Color success50 = Color(0xFFf0fdf1);
//   static const Color success100 = Color(0xFFddfbe1);
//   static const Color success200 = Color(0xFFbdf5c6);
//   static const Color success300 = Color(0xFF89ec9a);
//   static const Color success400 = Color(0xFF4cd964);
//   static const Color success500 = Color(0xFF27c041);
//   static const Color success600 = Color(0xFF1a9f31);
//   static const Color success700 = Color(0xFF187d2a);
//   static const Color success800 = Color(0xFF186326);
//   static const Color success900 = Color(0xFF165122);
//   static const Color success950 = Color(0xFF062d0e);

//   static const Color danger500 = Color(0xFFff3b30);
// }
